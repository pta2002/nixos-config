...
{ pkgs, lib, ... }:
let
  chromium-command = "${pkgs.chromium}/bin/chromium -a --kiosk --noerrdialogs --disable-infobars --no-first-run --ozone-platform=wayland --enable-features=OverlayScrollbar --start-maximized --force-dark-mode http://192.168.1.10:8123";
in
{
  imports = [ ];
